...
app.get('/api/dashboard/summary', async (req, res) => {
  ...
  WHERE to_char(m.trade_date,'YYYY-MM') = $1
    ...
    AND COALESCE(c.category_name,'') <> 'è³‡é‡‘ç§»å‹•'
    AND NOT (m.memo LIKE '%å‰Šé™¤æ¸ˆã¿%')
),
prev_total AS (
  SELECT COALESCE(SUM(
           CASE WHEN m.flow_type='IN'  THEN m.jpy_amount
      ...
      COALESCE((
        SELECT SUM(CASE WHEN me.flow_type='IN'  THEN me.jpy_amount
         ...
         WHERE date_trunc('month', me.trade_date) = date_trunc('month', m.month_start)
           ${segFilterInner}
           AND COALESCE(c.category_name,'') <> 'è³‡é‡‘ç§»å‹•'
AND NOT (me.memo LIKE '%å‰Šé™¤æ¸ˆã¿%')
      ),0) AS total
    FROM months m
    ORDER BY month;
  `;
...
app.get('/api/rates/monthly', async (req, res) => {
  try {
    ...
    const sql = `
      SELECT to_char(month, 'YYYY-MM') AS ym, currency, avg_rate
      FROM app.fx_rate_monthly
      WHERE EXTRACT(YEAR FROM month) = $1::int
...
app.get('/api/assets', async (req, res) => {
  ...
  const sql = `
   ...
   LEFT JOIN app.manual_entry_category c ON c.id = m.category_id
    ...
    agg AS (
      SELECT account_name,
             -- segment ã¯è¡¨ç¤ºç”¨(ä»£è¡¨å€¤)ã€‚åŒä¸€å£åº§ã§æ··åœ¨ã™ã‚‹å ´åˆã¯æœ€é »å€¤ã«ã—ãŸã„ãŒã€ã¾ãšã¯æœ€å¤§ä»¶æ•°ã®æ–¹ã‚’æŽ¡ç”¨
...
for (const r of records) {
  ...
  const tradeDate        = pick(r, ["trade_date", "date", "csvdate", "\ufeffæ—¥ä»˜", "æ—¥ä»˜"]) || null;
  const accountName      = pick(r, ["account_name", "account", "å£åº§ãƒ»ã‚¦ã‚©ãƒ¬ãƒƒãƒˆå"]) || null;
  const flowType         = (pick(r, ["flow_type", "type", "ã‚¿ã‚¤ãƒ—"]) || "IN").toUpperCase();
  const categoryName     = pick(r, ["category_name", "category", "ã‚«ãƒ†ã‚´ãƒª1", "ã‚«ãƒ†ã‚´ãƒª"]) || null;
  const amountNum        = toNum(pick(r, ["amount", "é‡‘é¡"]));
  const currencyCode     = (pick(r, ["currency", "curr", "é€šè²¨", "\ufeffé€šè²¨", "Currency"]) || "").toUpperCase() || null;
  const rateNum          = toNum(pick(r, ["rate", "ãƒ¬ãƒ¼ãƒˆ"]));
  const jpyAmountNum     = toNum(pick(r, ["jpy_amount", "å††å»ºã¦é‡‘é¡"]));
  // ★ 修正: content と memo を分離
  const contentText      = pick(r, ["content", "å†…å®¹"]) || null;
  const memoText         = pick(r, ["memo", "ãƒ¡ãƒ¢"]) || null;
  const segmentVal       = pick(r, ["segment", "åŒºåˆ†"]) || null;

  await client.query(
    `INSERT INTO staging.csv_import_raw
     (source_file, row_num, trade_date, account_name, flow_type, category_name, amount, currency, rate, jpy_amount, content, memo, segment)
    ...
    [
      ...
      rateNum,
      jpyAmountNum,
      contentText,  // ★ 追加: content を11番目に
      memoText,     // ★ 変更: 11番目 → 12番目に
      segmentVal,   // ★ 変更: 12番目 → 13番目に
    ]
  );
}
    ...
    await client.query(`
      ...
      SELECT
        ...
        CASE
          ...
          WHEN upper(replace(btrim(sd.currency),' ','')) IN ('å††','YEN','JPN') THEN 'JPY'
        ...
        COALESCE(
          ...
          END,
          CASE
            WHEN upper(trim(replace(sd.currency,' ',''))) IN ('JPY','å††','YEN','JPN') THEN 1
            ...
            ELSE COALESCE((
              SELECT rate_month_avg
               ...
               WHERE currency = CASE
                                  ...
                                  WHEN upper(trim(replace(sd.currency,' ',''))) IN ('JPY','å††','YEN','JPN') THEN 'JPY'
                                  ELSE upper(trim(replace(sd.currency,' ','')))
                                END
        ...
        CASE sd.segment
  WHEN 'æ³•äººè³‡ç”£' THEN 'corporate'
  WHEN 'å€‹äººè³‡ç”£' THEN 'personal'
  WHEN 'æ³•äººæŠ•è³‡' THEN 'corporate_invest'
  WHEN 'å€‹äººæŠ•è³‡' THEN 'personal_invest'
  ELSE NULL
...
app.post("/api/manual-entry", async (req, res) => {
  ...
  try {
    ...
    const normalizeFlow = (raw) => {
      ...
      if (s === "IN") return "IN";
      if (s === "OUT") return "OUT";
      if (s === "CF" || s === "CASHFLOW" || s === "ã‚­ãƒ£ãƒƒã‚·ãƒ¥ãƒ•ãƒ­ãƒ¼") return "ã‚­ãƒ£ãƒƒã‚·ãƒ¥ãƒ•ãƒ­ãƒ¼";
      return null;
    };
    const normalizeSegment = (raw) => {
      const s = normStr(raw);
      if (["corporate", "æ³•äººè³‡ç”£"].includes(s)) return "corporate";
      if (["personal", "å€‹äººè³‡ç”£"].includes(s)) return "personal";
      if (["corporate_invest", "æ³•äººæŠ•è³‡"].includes(s)) return "corporate_invest";
      if (["personal_invest", "å€‹äººæŠ•è³‡"].includes(s)) return "personal_invest";
      return null;
    };
    ...
    if (!flowType) return res.status(400).json({ error: "flow_type must be IN/OUT/ã‚­ãƒ£ãƒƒã‚·ãƒ¥ãƒ•ãƒ­ãƒ¼" });
    ...
    const sourceHash = crypto
      .createHash("md5")
      .update(`${source}:${tradeDate}:${accountName}:${flowType}:${amount}:${currency}:${rateUsed}:${content}:${memo}`)
      .digest("hex");
...
const sql = `
  ...
  ORDER BY created DESC   -- â† è¿½åŠ :æ–°è¦ãŒã‚ã‚Œã°ãã‚Œã‚’å„ªå…ˆ
  LIMIT 1
`;
const params = [
  tradeDate,
...
app.get("/api/manual-entry", async (req, res) => {
  ...
  WHERE date_trunc('month', m.trade_date) = $1::date
    ...
    AND (CASE WHEN $4 THEN COALESCE(c.category_name,'') <> 'è³‡é‡‘ç§»å‹•' ELSE TRUE END)
    ...
    const sqlSummary = `
      ...
      curr AS (
        ...
        WHERE date_trunc('month', m.trade_date) = date_trunc('month', p.m)
          ...
          AND (CASE WHEN p.ex THEN COALESCE(c.category_name,'') <> 'è³‡é‡‘ç§»å‹•' ELSE TRUE END)
      ),
      prev AS (
        SELECT
          COALESCE(SUM(CASE WHEN m.flow_type='IN'  THEN m.jpy_amount END),0) AS in_jpy,
...
async function upsertFiatMonthAverages(ym, symbols) {
  ...
  const avgJpyBase = await fetchFiatDailyJpyBase(ym, symbols);
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const upserted = [];
...
app.get('/api/rates', async (req, res) => {
  ...
  const months = Array.from({ length: 12 }, (_, i) => ({ month: `${i + 1}æœˆ` }));
